...
freeproc(struct proc *p)
{
  ...
  if(p->shared){
    uvmunmap(p->pagetable,(uint64)p->shared,p->shared_size/PGSIZE,p->owner_pid == p->pid);
  }
...
fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  if(p->shared){
    ...
    pte_t *pte;
    np->shared = p->shared;
    np->shared_size = p->shared_size;
    np->owner_pid = p->owner_pid;

    np->shared_size/=PGSIZE;
    va = (uint64)np->shared;
    i=0;

    for(i=0; i<np->shared_size; i++,va+=PGSIZE){
      
      pte = walk(p->pagetable, (uint64)va, 0);
      pa = PTE2PA(*pte);

      if(mappages(np->pagetable,(uint64) va, PGSIZE, (uint64)pa, PTE_FLAGS(*pte)) < 0){
        uvmunmap(p->pagetable,(uint64)np->shared,i,1);
        break;
      }
    }
...
smem(char * addr, int n){
  ...
  int i;
  void * pa;
  //check if n is multiple of 4096
  if(n % PGSIZE != 0)
    return -1;
  
  //check ig addr is page aligned
  ...
  for(i=0; i<n; i++){
    pa = kalloc();
    if(pa == 0){
      uvmunmap(p->pagetable,(uint64)addr,i,1);
      return -1;    
    }

    memset(pa, 0, PGSIZE);
    if(mappages(p->pagetable,(uint64)addr+i*PGSIZE,PGSIZE,(uint64)pa,PTE_W|PTE_R|PTE_U) < 0){
      kfree(pa);
      uvmunmap(p->pagetable,(uint64)addr,i,1);
      return -1;
    }
